            ...
            <div className="space-y-2">
                {sessions.map((session) => (
                    <button
                        ...
                        className={`w-full text-left p-3 rounded-lg transition-colors ${activeChatId === session.id ? "bg-blue-50 border-blue-100" : "hover:bg-gray-100"
                            }`}
                    >
    ...
    return (
        <div className="bg-white border-t border-gray-100 p-4 w-full">
            <div className="relative flex items-center w-full mx-auto">
                ...
                <button
                    onClick={() => onSend(isYtEnabled)} // Pass the YouTube toggle state when sending
                    disabled={!value.trim()}
                    className={`absolute right-3 p-1.5 rounded-full ${value.trim() ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-400"
                        } transition-colors`}
                >
                    <Send size={18} />
                </button>
